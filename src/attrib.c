...
static const struct innate {
  ...
  wiz_abil[] = {{15, &(HWarning), "sensitive", ""}, {17, &(HTeleport_control), "controlled", "uncontrolled"}, {0, 0, 0, 0}},
  ...
  hob_abil[] = {{1, &(HStealth), "", ""}, {7, &(HFast), "quick", "slow"}, {0, 0, 0, 0}},
...
#ifdef DEBUG
		pline("%s, %s AEXE = %d",
		      (i == A_STR) ? "Str" : (i == A_WIS) ? "Wis" : (i == A_DEX) ? "Dex" : "Con",
...
static void exerper(void) {
	...
	if (!(moves % 5)) {
		...
		if ((Wounded_legs && !u.usteed) || Fumbling || HStun) exercise(A_DEX, false);
	}
}
	...
	if (moves >= context.next_attrib_check && !multi) {
		...
		for (i = 0; i < A_MAX; AEXE(i++) /= 2) {
			...
			pline("exerchk: testing %s (%d).",
			      (i == A_STR) ? "Str" : (i == A_WIS) ? "Wis" : (i == A_DEX) ? "Dex" : "Con", AEXE(i));
#endif
			/*
		...
		context.next_attrib_check += rn1(2000, 2000);
#ifdef DEBUG
...
schar acurr(int x) {
	...
	if (x == A_STR) {
		...
		int bonus = tmp - base;
		boolean nobonus = (uarmg && uarmg->otyp == GAUNTLETS_OF_POWER && uarm && uarm->otyp == ROBE_OF_WEAKNESS);
