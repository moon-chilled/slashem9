...
bool is_lava(int x, int y) {
	...
	if (!isok(x, y)) return false;
	ltyp = levl[x][y].typ;
	if (ltyp == LAVAPOOL || (ltyp == DRAWBRIDGE_UP && (levl[x][y].drawbridgemask & DB_UNDER) == DB_LAVA)) return true;
	return false;
}
...
bool is_ice(int x, int y) {
	...
	if (!isok(x, y)) return false;
	ltyp = levl[x][y].typ;
	if (ltyp == ICE || (ltyp == DRAWBRIDGE_UP && (levl[x][y].drawbridgemask & DB_UNDER) == DB_ICE)) return true;
	return false;
}
	...
	if (is_u(etmp)) {
		if (how == DROWNING) {
			...
			if (!killer.name.len) {
				...
				killer.name = nhsdupz("falling drawbridge");
			}
			done(how);
			/* So, you didn't die */
...
static void do_entity(struct entity *etmp) {
	...
	if (is_db_wall(etmp->ex, etmp->ey)) {
		...
		if (!e_survives_at(etmp, etmp->ex, etmp->ey)) {
			killer.format = KILLED_BY_AN;
			killer.name = nhsdupz("closing drawbridge");
		...
		killer.name = nhsdupz("fell from a drawbridge");
...
void destroy_drawbridge(int x, int y) {
		...
		if (!automiss(etmp2)) {
			if (e_inview)
				pline("%s blown apart by flying debris.",
				      E_phrase(etmp2, "are"));
			killer.format = KILLED_BY_AN;
			killer.name = nhsdupz("exploding drawbridge");
	...
	if (etmp1->edata) {
		...
		if (e_missed(etmp1, true)) {
			...
			killer.format = KILLED_BY_AN;
			killer.name = nhsdupz("collapsing drawbridge");
