...
static int thrwmm(struct monst *magr, struct monst *mdef) {
			...
			if (mdef->mhp < 1) {
				if (canseemon(mdef))
					pline("%s is %s!", Monnam(mdef),
					      (nonliving(mdef->data) || !canspotmon(mdef)) ? "destroyed" : "killed");
				mondied(mdef);
	...
	if (canseemon(magr)) {
		...
		pline("%s %s %s!", Monnam(magr),
		      m_shot.s ? is_bullet(obj) ? "fires" : "shoots" : "throws", onm);
		...
		physical:
			...
			if (mattk->aatyp == AT_KICK && thick_skinned(pd)) {
				 ...
				 */
					...
					if (otmp->otyp == TORCH && otmp->lamplit && !resists_fire(mdef)) {
						if (!Blind) {
			...
			if ((mdef->misc_worn_check & W_ARMH) && rn2(8)) {
				if (vis) {
					...
					pline("%s %s blocks %s attack to %s head.",
					      buf, helm_simple_name(which_armor(mdef, W_ARMH)),
...
assess_dmg:
	if ((magr->mhp -= tmp) <= 0) {
		/* get experience from spell creatures */
		if (mdef->uexp)
			mon_xkilled(magr, "", (int)mddat->mattk[i].adtyp);
		else
