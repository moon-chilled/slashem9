...
die:
	...
	if (how <= GENOCIDED && iflags.in_dumplog) {
		dump_redirect(true);
		genl_outrip(0, how);
		dump_redirect(false);
	}
	...
	sprintf(pbuf, "and %ld piece%s of gold, after %ld move%s.", umoney, plur(umoney), moves, plur(moves));
	dump_forward_putstr(endwin, 0, pbuf, done_stopprint);
...
static void dump_everything(int how, time_t when) {
	...
	switch_graphics(ASCII_GRAPHICS); /* revert to default symbol set */

	putstr(0, 0, full_version_string_tmp());
	putstr(0, 0, "");
	...
	*/
	...
	display_inventory(NULL, true);
	container_contents(invent, true, true);
	enlightenment((how >= PANICKED) ? 1 : 2);
	putstr(0, 0, "");
