...
#define Q_set_boolopt s7_make_signature(s7, 3, s7_make_symbol(s7, "null?"), s7_make_signature(s7, 2, s7_make_symbol(s7, "string?"), s7_make_symbol(s7, "symbol?")), s7_make_symbol(s7, "boolean?"))
	...
	for (usize i = 0; boolopt[i].name; i++) {
		if (!strcmp(oname, boolopt[i].name)) {
			assign_boolopt(boolopt + i, value, true/*TODO set initial properly*/);
			return s7_nil(s7);
		}
	}
...
void s9s7_init(void) {
	s7 = s7_init();
	enforce(s7_define_typed_function(s7, "s9-option-set-boolopt", set_boolopt, 1, 2, false, H_set_boolopt, Q_set_boolopt));
	enforce(s7_define_typed_function(s7, "s9-option-add-menucolor", add_menucolor, 2, 2, false, H_add_menucolor, Q_add_menucolor));
}

void s9s7_load_options(const char *fname) {
	s7_pointer env = s7_inlet(s7, s7_list(s7, 2, s7_make_symbol(s7, "s9-env"), s7_make_symbol(s7, "options")));
	s7_load_with_environment(s7, fname, env);
}
