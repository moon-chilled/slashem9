...
static void mkshop(void) {
	...
	if (i < 0) { /* shoptype not yet determined */
		 ...
		 */
		if (isbig(sroom) && (shtypes[i].symb == WAND_CLASS || shtypes[i].symb == SPBOOK_CLASS)) i = 0;
	}
	...
	for (sx = sroom->lx; sx <= sroom->hx; sx++)
		for (sy = sroom->ly; sy <= sroom->hy; sy++) {
			...
			switch (type) {
				...
				case FUNGUSFARM:
					if (!rn2(3))
						mksobj_at(SLIME_MOLD, sx, sy, true, false);
					break;
				case MIGOHIVE:
					switch (rn2(10)) {
						case 9:
...
static struct permonst *morguemon(void) {
	...
	return (i < 20) ? &mons[PM_GHOST] : (i < 40) ? &mons[PM_WRAITH] : mkclass(S_ZOMBIE, 0);
}
...
static struct permonst *fungus() {
	...
	switch (i) {
		...
		case 6:
			...
			break;
		case 7:
			mtyp = PM_VIOLET_FUNGUS;
			break;
		case 8:
			mtyp = PM_BLUE_JELLY;
			break;
		case 9:
			mtyp = PM_DISGUSTING_MOLD;
			break;
		case 10:
			mtyp = PM_BLACK_MOLD;
			break;
		...
		case 12:
			mtyp = PM_SPOTTED_JELLY;
			break;
		case 13:
			mtyp = PM_BROWN_PUDDING;
			break;
				...
				if (!OBJ_AT(sx, sy) &&
					...
					if ((sx + sy) % 2) {
						...
						if (!eelct || !rn2(4)) {
							...
							makemon(rn2(5) ? &mons[PM_GIANT_EEL] : rn2(2) ? &mons[PM_PIRANHA] : &mons[PM_ELECTRIC_EEL],
								sx, sy, NO_MM_FLAGS);
							eelct++;
						}
