...
#define m_flee(m) \
	if (fleetim && !m->iswiz) { monflee(m, fleetim, false, false); }
	...
	switch (m.has_defense) {
		...
		case MUSE_WAN_TELEPORTATION_SELF:
			if ((mtmp->isshk && inhishop(mtmp)) || mtmp->isgd || mtmp->ispriest) return 2;
			m_flee(mtmp);
...
try_again:
	switch (rn2(8 + (difficulty > 3) + (difficulty > 6) +
		...
		case 5:
			return (mtmp->data != &mons[PM_PESTILENCE]) ?
				       POT_FULL_HEALING :
				       POT_SICKNESS;
		case 7:
...
#define nomore(x) \
	...
	for (obj = mtmp->minvent; obj; obj = obj->nobj) {
		...
		if (obj->otyp == SCR_EARTH && ((helmet && is_metallic(helmet)) || mtmp->mconf || amorphous(mtmp->data) || passes_walls(mtmp->data) || noncorporeal(mtmp->data) || unsolid(mtmp->data) || !rn2(10)) && dist2(mtmp->mx, mtmp->my, mtmp->mux, mtmp->muy) <= 2 && mtmp->mcansee && haseyes(mtmp->data) && !Is_rogue_level(&u.uz) && (!In_endgame(&u.uz) || Is_earthlevel(&u.uz))) {
			m.offensive = obj;
...
boolean mon_reflects(struct monst *mon, const char *str) {
	...
	if (orefl && orefl->otyp == SHIELD_OF_REFLECTION) {
		...
		return true;
	} else if (mon->data == &mons[PM_DIAMOND_GOLEM] || mon->data == &mons[PM_SAPPHIRE_GOLEM] || mon->data == &mons[PM_CRYSTAL_GOLEM]) {
...
boolean ureflects(const char *fmt, const char *str) {
	...
	if (EReflecting & W_ARMS) {
		...
		return true;
	} else if (youmonst.data == &mons[PM_DIAMOND_GOLEM] || youmonst.data == &mons[PM_SAPPHIRE_GOLEM] || youmonst.data == &mons[PM_CRYSTAL_GOLEM]) {
		if (fmt && str)
			pline(fmt, str, "body");
		return true;
	}
