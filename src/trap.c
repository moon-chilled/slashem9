		...
		switch (rn2(5)) {
			case 0:
				...
				if (item) {
					...
					sprintf(buf, "%s %s", materialnm[mat_idx], helm_simple_name(uarmh));
				}
...
struct trap *maketrap(int x, int y, int typ) {
	...
	switch (typ) {
		case STATUE_TRAP: { /* create a "living" statue */
			...
			statue = mkcorpstat(STATUE, NULL, &mons[rndmonnum()], x, y, CORPSTAT_NONE);
 ...
 * or pick-axe.
 */
struct monst *activate_statue_trap(struct trap *trap, xchar x, xchar y, bool shatter) {
			...
			if (trap->once && trap->tseen && !rn2(15)) {
				...
				if (uarmh) {
					if (is_metallic(uarmh)) {
						...
						pline("%s not protect you.", Yobjnam2(uarmh, "does"));
					}
				}
			...
			switch (rn2(5)) {
				case 0:
					pline("%s you on the %s!", A_gush_of_water_hits,
					      body_part(HEAD));
					rust_dmg(uarmh, helm_simple_name(uarmh), 1, true, &youmonst);
					break;
				case 1:
...
void mon_drain_en(struct monst *mtmp, int n) {
			...
			case RUST_TRAP: {
				...
				switch (rn2(5)) {
					case 0:
						...
						rust_dmg(target, helm_simple_name(target), 1, true, mtmp);
						break;
					case 1:
						if (in_sight)
				...
				if (!rn2(30)) {
					...
					if (in_sight)
						pline("%s is caught in a magical explosion.",
						      Monnam(mtmp));
					if (thitm(0, mtmp, NULL, rnd(10), false))
						trapkilled = true;
					else {
						if (in_sight)
...
static void domagictrap(void) {
	...
	if (fate < 10) {
		...
		switch (fate) {
			...
			/* odd feelings */
			case 13:
				pline("A shiver runs up and down your %s!",
				      body_part(SPINE));
				break;
			case 14:
				You_hear(Hallucination ?
...
bool water_damage(struct obj *obj, bool force, bool here) {
	...
	for (; obj; obj = otmp) {
		...
		if (obj->otyp == CAN_OF_GREASE && obj->spe > 0) {
		...
		} else if (obj->oclass == ARMOR_CLASS && obj == uarm && uarmc && uarmc->otyp == OILSKIN_CLOAK && (!uarmc->cursed || rn2(3))) {
			continue;
		} else {
			...
			switch (obj->oclass) {
				...
				case POTION_CLASS:
					...
					break;
				case GEM_CLASS:
					if (level.flags.lethe && (obj->otyp == LUCKSTONE || obj->otyp == LOADSTONE || obj->otyp == HEALTHSTONE || obj->otyp == TOUCHSTONE))
						obj->otyp = FLINT;
					break;
				case TOOL_CLASS:
					if (level.flags.lethe) {
				...
				case WAND_CLASS:
				case RING_CLASS:
					if (level.flags.lethe && (obj->oclass == WEAPON_CLASS || obj->oclass == ARMOR_CLASS || obj->oclass == WAND_CLASS || obj->oclass == RING_CLASS || is_weptool(obj))) {
						/* Shift enchantment one step closer to 0 */
						if (obj->spe > 0) drain_item(obj);
					}

					/* Magic markers run... */
					if (level.flags.lethe && obj->otyp == MAGIC_MARKER) {
						obj->spe -= (3 + rn2(10));
...
bool drown(void) {
	...
	if (level.flags.lethe) {
		/* Bad idea */
		pline("You feel the sparkling waters of the Lethe sweep away your "
		      "cares!");
		forget(25);
	}
...
static int disarm_holdingtrap(struct trap *ttmp) {
	...
	if ((mtmp = m_at(ttmp->tx, ttmp->ty)) != 0) {
		mtmp->mtrapped = 0;
		pline("You remove %s %s from %s.", ttmp->madeby_u ? "your" : "the",
		      (ttmp->ttyp == BEAR_TRAP) ? "bear trap" : "webbing",
		      mon_nam(mtmp));
		reward_untrap(ttmp, mtmp);
	} else {
		if (ttmp->ttyp == BEAR_TRAP) {
			pline("You disarm %s bear trap.", ttmp->madeby_u ? "your" : "the");
			cnv_trap_obj(BEARTRAP, 1, ttmp);
		} else { /* if (ttmp->ttyp == WEB) */
			pline("You succeed in removing %s web.", ttmp->madeby_u ? "your" : "the");
			deltrap(ttmp);
		}
	}
...
static int disarm_landmine(struct trap *ttmp) {
	...
	if (fails < 2) return fails;
	pline("You disarm %s land mine.", ttmp->madeby_u ? "your" : "the");
	cnv_trap_obj(LAND_MINE, 1, ttmp);
	return 1;
}

/* Paul Sonier */
static int disarm_rust_trap(struct trap *ttmp) {
	xchar trapx = ttmp->tx, trapy = ttmp->ty;
	int fails = try_disarm(ttmp, false);

	if (fails < 2) return fails;
	pline("You disarm the water trap!");
	deltrap(ttmp);
...
/* Paul Sonier */
static int disarm_fire_trap(struct trap *ttmp) {
	int fails;
	struct obj *obj;
	boolean bad_tool;
	...
	if (obj->otyp == POT_OIL) {
		pline("Your potion of oil explodes!");
	...
	useup(obj);
	makeknown(POT_WATER);
	pline("You manage to extinguish the pilot light!");
...
static int disarm_shooting_trap(struct trap *ttmp, int otyp) {
	...
	if (fails < 2) return fails;
	pline("You disarm %s trap.", ttmp->madeby_u ? "your" : "the");
...
int untrap(bool force) {
	...
	if ((ttmp = t_at(x, y)) && ttmp->tseen) {
		...
		if (deal_with_floor_trap) {
			...
			switch (ttmp->ttyp) {
				...
				default:
					pline("You cannot disable %s trap.", (u.dx || u.dy) ? "that" : "this");
					return 0;
			}
		}
	} /* end if */
	...
	if (!u.dx && !u.dy) {
		...
		pline(trap_skipped ? "You find no other traps here." : "You know of no traps here.");
		return 0;
	}
	...
	if ((levl[x][y].doormask & D_TRAPPED && (force ||
...
static boolean thitm(int tlev, struct monst *mon, struct obj *obj, int d_override, boolean nocorpse) {
	int strike;
	boolean trapkilled = false;
