...
static boolean no_bones_level(d_level *lev) {
	...
	return ((sptr = Is_special(lev)) != 0 && !sptr->boneid) || !dungeons[lev->dnum].boneid
	       /* no bones on the last or multiway branch levels */
	       /* in any dungeon (level 1 isn't multiway).       */
	       || Is_botlevel(lev) || (Is_branchlev(lev) && lev->dlevel > 1) || (lev->dlevel < 2) /* no bones on 1st level */
...
static void resetobjs(struct obj *ochain, boolean restore) {
	...
	for (otmp = ochain; otmp; otmp = otmp->nobj) {
		...
		if (((otmp->otyp != CORPSE || otmp->corpsenm < SPECIAL_PM) && otmp->otyp != STATUE) && (!otmp->oartifact ||
													(restore && (exist_artifact(otmp->otyp, ONAME(otmp)) || is_quest_artifact(otmp))))) {
			otmp->oartifact = 0;
			otmp->onamelth = 0;
			*ONAME(otmp) = '\0';
