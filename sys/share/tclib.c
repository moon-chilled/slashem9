...
/* size of entbuf must be at least [TCBUFSIZ] */
...
static int tc_store(const char *trm, const char *ent) {
	...
	if (!ent || !*ent || !trm || !*trm || (col = index(ent, ':')) == 0)
		return 0;
	strcpy(tc_entry, trm);
	if (((bar = index(ent, '|')) != 0 && bar < col) || ((long)(n = strlen(trm)) == (long)(col - ent) && strncmp(ent, trm, n) == 0))
		strcat(tc_entry, col);
	else if (*ent == ':')
		strcat(tc_entry, ent);
	else
...
static const char *tc_field(const char *field, const char **tc_end) {
	...
	while (p < end) {
		if ((p = index(p, ':')) == 0)
			break;
		++p;
		if (p[0] == field[0] && p[1] == field[1] && (p[2] == ':' || p[2] == '=' || p[2] == '#' || p[2] == '@'))
			break;
	}
	if (tc_end) {
		if (p) {
...
void tputs(const char *string, int range, int (*output_func)()) {
	int c, num = 0;
	const char *p = string;
