...
void
start_server(char *prgname, void *read_h, void *write_h)
{
    int nb;
    char *s;
...
main(int argc, char **argv)
{
    ...
    nargv = (char **)alloc((argc + 2) * sizeof(char *));
    /*
     * Using the p variants of spawn should be equivalent to doing this
     * but in practice glib is able to find slashem if it is installed
     * in the same directory as where we were run from even if this
     * directory is not in the path. spawnp() fails in this case which
     * is odd considering that this is the documented behaviour of
     * CreateProcess().
     */
    ...
    start_server(argv[0], (void *)from_game_h[0], (void *)to_game_h[1]);
    /* start_server only returns if it failed */
    exit(1);
}
...
main(int argc, char **argv)
{
	...
	close(to_game[0]);
	close(from_game[1]);
	start_server(argv[0], (void *)from_game[0], (void *)to_game[1]);
	/* start_server only returns if it failed */
	exit(1);
    }
}
