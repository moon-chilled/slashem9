...
int curses_display_nhmenu(winid wid, int how, menu_item **_selected) {
	...
	*_selected = NULL;
	...
	if (num_chosen > 0) {
		selected = alloc(num_chosen * sizeof(menu_item));
		count = 0;
...
menu_get_accel(bool first) {
	...
	if (((next_letter < 'z') && (next_letter >= 'a')) || ((next_letter < 'Z') && (next_letter >= 'A')) ||
...
menu_display_page(nhmenu *menu, WINDOW *win, int page_num) {
	...
	while (menu_item_ptr != NULL) {
		...
		if (iflags.use_menu_color && (menu_color = get_menu_coloring((char *)menu_item_ptr->str, &color,
									     &attr))) {
			if (color != NO_COLOR) {
				curses_toggle_color_attr(win, color, NONE, ON);
			}
		...
		for (count = 0; count < num_lines; count++) {
			if (strlen(menu_item_ptr->str) > 0) {
				mvwprintw(win, menu_item_ptr->line_num + count + 1,
					  start_col, "%s", curses_break_str(menu_item_ptr->str, entry_cols, count + 1));
			}
		}
			...
			if (menu_item_ptr->identifier.a_void != NULL) {
				if (((curletter == menu_item_ptr->accelerator) &&
				     ((curpage == menu_item_ptr->page_num) ||
				      (!menu->reuse_accels))) ||
				    ((menu_item_ptr->group_accel) && (curletter ==
