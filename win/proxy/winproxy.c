...
void
	proxy_init_nhwindows(argcp, argv) int *argcp;
char **argv;
{
	int i, j, capc;
	char **capv;
...
void
	proxy_start_menu(window) int window;
{
...
static void
	proxy_print_layer(w, x, y, z, glyph) int w;
xchar x, y, z;
int glyph;
{
	int g;
...
static void
	proxy_flush_layers(w, clearing) int w;
int clearing;
{
...
int glyph;
{
	...
	if (w >= 0 && proxy_interface_mode & NHPROXY_EXT_IM_DISPLAY_LAYERS) {
		...
		if (level.flags.hero_memory) {
			if (lev->mem_obj)
				proxy_print_layer(w, x, y, 1, lev->mem_corpse ? body_to_glyph(lev->mem_obj - 1) : objnum_to_glyph(lev->mem_obj - 1));
			else
				proxy_print_layer(w, x, y, 1, NO_GLYPH);
			proxy_print_layer(w, x, y, 2, lev->mem_trap ? cmap_to_glyph(lev->mem_trap - 1 + MAXDCHARS) : NO_GLYPH);
			switch (lev->mem_bg) {
				case S_room:
				case S_corr:
				case S_litcorr:
...
void proxy_config_file_close(FILE *fp) {
	nhproxy_serv_config_file_close(fp);
}

static void
	win_proxy_errhandler(class, error) int class;
const char *error;
{
