...
static void
	cb_interface_mode(mode) long mode;
{
...
static void
	cb_free_player_choices(choices) struct nhproxy_cb_get_player_choices_res *choices;
{
	free(choices->races);
	free(choices->roles);
	free(choices);
...
static void
	cb_free_valid_selections(vs) struct nhproxy_cb_get_valid_selections_res *vs;
{
	free(vs->masks);
	free(vs);
}
...
struct nhproxy_serv_callbacks win_proxy_callbacks = {
	...
	(char NHPROXY_ *(NHPROXY_char))map_menu_cmd,
